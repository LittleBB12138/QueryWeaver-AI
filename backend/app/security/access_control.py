...
ALL_DATABASES = frozenset(str(table.get("database") or "demo_mock") for table in SCHEMA)
...
DEMO_MOCK_TABLES = frozenset(
    str(table["id"])
    for table in SCHEMA
    if table.get("database", "demo_mock") == "demo_mock"
)
...
class AccessController:
    ...
    ROLE_POLICIES = {
        ...
        "analyst": {
            "databases": frozenset({"demo_mock"}),
            "tables": DEMO_MOCK_TABLES,
        },
        "current_sales": {
    ...
    def filter_schema(scope: AccessScope) -> list[dict[str, Any]]:
        return [
            ...
            if scope.allows_table(
                str(table.get("database") or "demo_mock"),
                str(table["id"]),
            )
        ]
